import { formInitFields } from '$comps/esp/form/formInit'
...
export class Form {
	...
	constructor(formDefn) {}
	// constructor(formDefn) {
	// 	this.name = formDefn.name
	// 	this.label = formDefn?.Label
	// 	this.description = formDefn?.description
	// 	this.submitButtonLabel = formDefn?.submitButtonLabel
	// 	this.fields = formInitFields(formDefn)
	// }

	validateform(event: SubmitEvent, formDefn) {
		// set formData
	...
	validateField(field, formData): Validation {
		...
		if (!evalutedAccessTypes.has(field?.access)) {
			return fieldValid()
		}
		...
		if (field.pattern) {
			...
			if (!regex.test(fieldValue)) {
				const errorMsg =
					field.patternMsg || `The value you entered is not a valid "${field.label}".`
