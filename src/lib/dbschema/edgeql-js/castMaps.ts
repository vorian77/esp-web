...
export type scalarAssignableBy<T extends $.ScalarType> =
  ...
  T extends _default.$nonNegative ? _default.$nonNegative : 
...
type getSharedParentScalar<A, B> =
  ...
  A extends _fts.$ElasticLanguage ?
    ...
    never
  :
  A extends _default.$nonNegative ?
    B extends _default.$nonNegative ?
    B
    :
    never
  :
...
function getSharedParentScalar<A extends $.ScalarType, B extends $.ScalarType>(a: A, b: B): A | B {
  ...
  if (a.__name__ === "default::nonNegative") {
    if(b.__name__ === "default::nonNegative") {
      return b;
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`);
    }
